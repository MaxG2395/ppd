...
char *argv[];
{
	...
	if (getsockname(0, (struct sockaddr *)&saddr, &asize) == 0) {
		...
		_rpcpmstart = 1;
		proto = 0;
		openlog("bank", LOG_PID, LOG_DAEMON);
	} else {
#ifndef RPC_SVC_FG
		int size;
		int pid, i;
...
#endif
		sock = RPC_ANYSOCK;
		(void) pmap_unset(BANK_PROG, BANK_VERS);
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, BANK_PROG, BANK_VERS, bank_prog_1, proto)) {
			_msgout("unable to register (BANK_PROG, BANK_VERS, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, BANK_PROG, BANK_VERS, bank_prog_1, proto)) {
			_msgout("unable to register (BANK_PROG, BANK_VERS, tcp).");
			exit(1);
		}
	}
